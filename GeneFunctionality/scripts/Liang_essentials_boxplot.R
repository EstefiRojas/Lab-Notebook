...
# Load csv file
essentials_lncrna_data <- read.csv("../results/gRNA_essential_matches_no_introns_optimized.tsv", sep = "\t", header = TRUE)
essentials_lncrna_data_previous <- read.csv("../results/gRNA_essential_matches_optimized.tsv", sep = "\t", header = TRUE)


ess_d <- essentials_lncrna_data %>% filter(match_pc == "NO")
ess_d_excluded <- essentials_lncrna_data %>% filter(match_pc == "YES")
table(essentials_lncrna_data$match_pc)
table(essentials_lncrna_data$Essentiality)
...
filtered_df <- ess_d %>%
  ...
  slice_max(order_by = highest_prob, n = 1, with_ties = FALSE) %>%
...
plot_modified <- ggplot(data = filtered_df,
                        aes(x = Essentiality, y = highest_prob, 
