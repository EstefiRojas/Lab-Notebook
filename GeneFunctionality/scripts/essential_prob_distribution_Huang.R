...
# Load csv file
essentials_exon1_data <- read.csv("../data/Huang/processed/annotated_crRNAs_vs_exon1_prob.csv", header = TRUE)
essentials_exon2_data <- read.csv("../data/Huang/processed/annotated_crRNAs_vs_exon2_prob.csv", header = TRUE)


ess_e1_d <- essentials_exon1_data %>% select(Q_name_suffix,GeneID,TranscriptID,Probability_Functional,Essentiality_Status,coPARSE.lncRNA)
ess_e2_d <- essentials_exon2_data %>% select(Q_name_suffix,GeneID,TranscriptID,Probability_Functional,Essentiality_Status,coPARSE.lncRNA)

# Join the two datasets by Q_name and GeneID
ess_d_join <- full_join(ess_e1_d, ess_e2_d, by = c("Q_name_suffix", "GeneID", "Essentiality_Status"))

# Get the max probability and coPARSE.
ess_d_join <- ess_d_join %>% mutate(max_prob = pmax(Probability_Functional.x, Probability_Functional.y, na.rm = TRUE),
                                    coPARSE = ifelse(is.na(coPARSE.lncRNA.x),coPARSE.lncRNA.y,coPARSE.lncRNA.x))


table(ess_d_join$Essentiality_Status)

# Define the desired order for the categorical variable
essentiality_labels <- c("Non", "Essential")
coparse_labels <- c("Non", "Non-coPARSE", "coPARSE")

# Reorder the 'Essential_Status' column by converting it to a factor with specified levels
ess_d_join <- ess_d_join %>%
  mutate(Essential_Status = factor(Essentiality_Status, levels = c("Non-essential","Essential"), labels = essentiality_labels),
         coPARSE_category = case_when(
           Essentiality_Status == "Essential" & coPARSE == "Yes" ~ "coPARSE",
           Essentiality_Status == "Essential" & coPARSE == "No" ~ "Non-coPARSE",
           TRUE ~ "Non"
         ),
         coPARSE_category = factor(coPARSE_category, levels = coparse_labels))
...
ks_test_custom <- function(x, y) {
  ...
  label <- paste0(format(test$statistic, digits = 2), " (", test$p.value, ")")
...
# Calculate the number of records for each Essential_Status category.
legend_data <- filtered_df %>%
  count(coPARSE_category)

# Create a named vector for the new labels.
# The names of the vector are the original categories (e.g., "Essential")
# The values are the new labels with counts (e.g., "Essential (n=1)")
new_labels <- setNames(
  paste0(legend_data$coPARSE_category, "\n(n=", legend_data$n, ")"),
  legend_data$coPARSE_category
)
...
# Generate the Violin Plot
plot_modified <- ggplot(data = filtered_df,
                        aes(x = coPARSE_category, y = max_prob, fill = coPARSE_category)) +
...
# Define the pairwise comparisons to be performed
my_comparisons <- combn(coparse_labels, 2, simplify = FALSE)
...
# Define your reference group and comparison groups
reference_group <- "Non"
comparison_groups <- c("Non-coPARSE","coPARSE")

# Extract the data for the reference group
reference_data <- filtered_df %>%
  filter(coPARSE_category == reference_group) %>%
  pull(max_prob)
...
plot_with_text_stats <- plot_modified +
  ...
  geom_text(
    data = stats_labels,
    aes(x = coPARSE_category, y = y_position, label = label),
    inherit.aes = FALSE, 
    size = 9.5,
    color = "black",
    fontface = "bold"
