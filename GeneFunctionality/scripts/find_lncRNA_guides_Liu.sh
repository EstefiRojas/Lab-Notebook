#!/bin/bash
# ==============================================================================
# Script: find_lncRNA_guides_adapted.sh
...
#              The script filters out negative control gRNAs.
...
#   2. Make the script executable: chmod +x scripts/find_lncRNA_guides_adapted.sh
#   3. Run the script with your CSV file as argument:
#      ./scripts/find_lncRNA_guides_adapted.sh path/to/gRNA_protospacers.csv
#
# ==============================================================================
...
FINAL_REPORT="$RESULTS_DIR/gRNA_lncRNA_matches_Liu.tsv"
...
if [ -z "${1-}" ]; then
    ...
    echo "Usage: ./scripts/find_lncRNA_guides_Liu.sh <gRNA_protospacers.csv>"
    exit 1
fi
INPUT_GRNA_CSV="$1"
...
# ==============================================================================
echo "--- Step 3: Filtering gRNAs (excluding negative controls) and creating FASTA file ---"

rm -f "$FILTERED_GUIDES_FASTA" "$GRNA_METADATA_MAP"
touch "$FILTERED_GUIDES_FASTA"
touch "$GRNA_METADATA_MAP"

# Process the input CSV: filter out negative_control entries
# Generate unique gRNA IDs from Gene ID + sequence combination
awk 'BEGIN {FS=","; OFS="\t"; counter=1}
{
    ...
    # Filter out negative_control entries (case-insensitive)
    if (tolower($1) != "negative_control") {
        # Create a unique gRNA ID
        gRNA_id = $1 "_gRNA_" counter;
        counter++;
        
        # Write to FASTA file
        print ">" gRNA_id "\n" $2 >> "'"$FILTERED_GUIDES_FASTA"'"
        
        # Write to metadata map: gRNA_ID, Gene_ID, Sequence
        print gRNA_id, $1, $2 >> "'"$GRNA_METADATA_MAP"'"
    }
}' "$INPUT_GRNA_CSV"

FILTERED_COUNT=`grep -c "^>" "$FILTERED_GUIDES_FASTA" || true`
...
if [ "$FILTERED_COUNT" -eq 0 ]; then
    echo "No gRNAs passed the filter. Exiting."
    touch "$FINAL_REPORT"
    exit 0
fi

echo "Aligning gRNAs to the genome with BLAT (this may take a while)..."
blat -t=dna -q=dna "$GENOME_FASTA" "$FILTERED_GUIDES_FASTA" -stepSize=5 -repMatch=2253 -minScore=15 -minIdentity=0 "$BLAT_OUTPUT_PSL" -noHead
...
echo -e "gRNA_ID\tTarget_Gene_ID\tgRNA_Sequence\tgRNA_Chr\tgRNA_Start\tgRNA_End\tlncRNA_Feature_Type\tlncRNA_ENSG_ID\tlncRNA_Chr\tlncRNA_Start\tlncRNA_End" > "$FINAL_REPORT"
...
# 2. Process the intersected results from the left outer join
{
    ...
    print gRNA_id, gene_id_map[gRNA_id], sequence_map[gRNA_id], gRNA_chr, gRNA_start, gRNA_end, lncRNA_feature, lncRNA_ensg, lncRNA_chr, lncRNA_start, lncRNA_end
}' "$GRNA_METADATA_MAP" "$TEMP_INTERSECT" | sort | uniq >> "$FINAL_REPORT"

# Clean up intermediate files from the tmp directory (commented out for debugging)
#rm "$TMP_DIR"/*
