...
#              This script uses a structured project directory layout.
...
LNCRNA_BED="$TMP_DIR/lncrna_genes.bed"
...
# ==============================================================================
# --- STEP 2: Create a BED file of lncRNA Gene Coordinates ---
# ==============================================================================
echo "--- Step 2: Creating a BED file of lncRNA gene coordinates ---"

# Parse the dedicated lncRNA GTF to get the genomic coordinates for each gene.
awk 'BEGIN{OFS="\t"} $3 == "gene" {
    attributes = $0;
    sub(/.*gene_id "/, "", attributes);
    sub(/".*/, "", attributes);
    print $1, $4-1, $5, attributes, "0", $7
}' "$LNCRNA_GTF" > "$LNCRNA_BED"

echo "Created BED file for lncRNAs: '$LNCRNA_BED'"
LNCRNA_GENE_COUNT=`wc -l < "$LNCRNA_BED"`
echo "$LNCRNA_GENE_COUNT lncRNA genes found."
echo ""
...
}
{
    ...
    if (FNR > 1 && ($2 in essential_genes)) {
        ...
        print $1, essential_genes[$2], $2 >> "'"$GRNA_METADATA_MAP"'"
    }
...
echo "Intersecting gRNA alignments with lncRNA regions..."
bedtools intersect -a "$BLAT_OUTPUT_BED" -b "$LNCRNA_BED" -loj > "$TEMP_INTERSECT"

echo "Formatting final report..."
echo -e "gRNA_ID\tEssentiality\tTarget_Gene_ID\tgRNA_Chr\tgRNA_Start\tgRNA_End\tlncRNA_ENSG_ID\tlncRNA_Chr\tlncRNA_Start\tlncRNA_End" > "$FINAL_REPORT"
...
# 2. Process the intersected results
{
    gRNA_id = $4;
    gRNA_chr = $1;
    gRNA_start = $2;
    ...
    print gRNA_id, essential_map[gRNA_id], target_gene_map[gRNA_id], gRNA_chr, gRNA_start, gRNA_end, lncRNA_ensg, lncRNA_chr, lncRNA_start, lncRNA_end
