...
LNCRNA_FASTA_URL="https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_49/gencode.v49.long_noncoding_RNAs.fa.gz"
...
LNCRNA_FASTA_NAME="gencode.v49.lncRNA_transcripts.fa"
LNCRNA_GTF_NAME="gencode.v49.lncRNA_transcripts.gtf"

GENOME_FASTA="$REF_DIR/$GENOME_FASTA_NAME"
LNCRNA_FASTA="$REF_DIR/$LNCRNA_FASTA_NAME"
...
FINAL_REPORT="$RESULTS_DIR/gRNA_lncRNA_antisense_matches.tsv"
...
if [ ! -f "$LNCRNA_FASTA" ]; then
    echo "Downloading fasta lncRNA annotations to $REF_DIR..."
    wget -O ${LNCRNA_FASTA}.gz "$LNCRNA_FASTA_URL"
    echo "Decompressing annotations..."
    gunzip ${LNCRNA_FASTA}.gz
    echo "Downloading gtf lncRNA annotations to $REF_DIR..."
    wget -O ${LNCRNA_GTF}.gz "$LNCRNA_GTF_URL"
    echo "Decompressing annotations..."
    gunzip ${LNCRNA_GTF}.gz

    add_strand_to_fasta.sh ${LNCRNA_FASTA} ${LNCRNA_GTF} > ${LNCRNA_FASTA}.strand.gtf
    mv ${LNCRNA_FASTA}.strand.gtf ${LNCRNA_FASTA}
    rm -rf ${LNCRNA_FASTA}.strand.gtf

else
    echo "lncRNA FASTA file '$LNCRNA_FASTA' already exists. Skipping download."
fi
...
# ==============================================================================
echo "--- Step 4: Running BLAT against the Whole lncRNA Genome Annotations ---"
...
blat -t=dna -q=dna "$LNCRNA_FASTA" "$FILTERED_GUIDES_FASTA" -stepSize=5 -repMatch=2253 -minScore=20 -minIdentity=0 "$BLAT_OUTPUT_PSL".tmp -noHead
echo "BLAT alignment complete."

echo "Filtering for antisense hits only (strand field contains '-')..."
# Filter to keep only lines where column 9 (strand) starts with '-' (reverse complement)
awk '$9 ~ /^-/' "$BLAT_OUTPUT_PSL.tmp" > "$BLAT_OUTPUT_PSL"
REVERSE_HIT_COUNT=$(wc -l < "$BLAT_OUTPUT_PSL")
echo "Found $REVERSE_HIT_COUNT reverse strand hits. Output saved to '$BLAT_OUTPUT_PSL'"
echo ""
...
awk 'BEGIN{OFS="\t"} {print $14, $10, $9}' "$BLAT_OUTPUT_PSL" > "$BLAT_OUTPUT_BED"
...
echo -e "gRNA_ID\tEssentiality\tTarget_Gene_ID\tgRNA_Sequence\tlncRNA_ENSG_ID\tStrand" > "$FINAL_REPORT"

# Note the swapped file order: BLAT_OUTPUT_BED is now FIRST.
awk 'BEGIN{FS="\t"; OFS="\t"}
