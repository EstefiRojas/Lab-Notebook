...
# Script Name: fetch_ensembl_exon_data_v2.sh
...
#
# Usage: ./fetch_ensembl_exon_data_v2.sh regions_to_extract.csv outname.csv
...
process_transcript() {
    ...
    name=$(echo "$ensembl_response" | jq -r '.display_name // "NA"')
    fields=$(echo "$ensembl_response" | jq -r '[.seq_region_name // "NA", .start // "NA", .end // "NA"] | @csv')
    
    # If start coordinate is empty or NA, try archive API
    if [[ "$(echo "$fields" | cut -d',' -f2)" == '"NA"' || "$(echo "$fields" | cut -d',' -f2)" == '""' ]]; then
        log "INFO" "No data found in current API for ${trans_id}, trying archive API"
        base_url="https://may2024.rest.ensembl.org"
        ensembl_response=$(make_api_request "${base_url}/lookup/id/$(echo "${trans_id%.*}")?expand=1;content-type=application/json")
        
        # Process response and extract fields again
        name=$(echo "$ensembl_response" | jq -r '.display_name // "NA"')
        fields=$(echo "$ensembl_response" | jq -r '[.seq_region_name // "NA", .start // "NA", .end // "NA"] | @csv')
    fi
    
    # Get exon fields after we have the final response
    exon1_fields=$(echo "$ensembl_response" | jq -r '[ .Exon[0].start // "NA", .Exon[0].end // "NA"] | @csv')
    exon2_fields=$(echo "$ensembl_response" | jq -r '[ .Exon[1].start // "NA", .Exon[1].end // "NA"] | @csv')
