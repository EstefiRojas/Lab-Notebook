...
lncrna_coords_data <- lncrna_coords_data |>
  ...
  mutate(min_pval = 
           tryCatch( 
             ...
             error = function(e) 1 # If no associations or variants found, return 1
           )
  )
...
lncrna_coords_data |> 
  mutate(Type = "lncRNA") |>
  ggplot(aes(x = min_pval, fill = Type, colour = Type)) +
  geom_histogram(alpha = 0.6, bins = 120) +
  ...
  theme(axis.text.x = element_text(size = 32, angle = 90),
...
ggsave(file.path("../results/latest1000all/GWAS_min_pval_logscale", paste0("lncRNA", "_", "GWAS_min_pval", "_histogram.png")), scale = 3, width = 10, height = 6, bg = "white", dpi = 300)
...
associations <- associations_data@associations
