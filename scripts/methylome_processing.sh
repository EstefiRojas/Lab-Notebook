...
# Variables
data_path=../data/methylome_feature/by_biosample_class
beds_path=../data/epigenetic_data/methylome
...
#if [ -f "$beds_path"/*.bed.gz ]; then
#    for file in "$beds_path"/*.bed.gz; do
#        gunzip "$file"
#    done
#fi
...
for file in $(find "$beds_path" -type f -print | grep .bed.gz); do
    ...
    if [ ! -f "$data_path"/sorted_beds/"${filename%.bed}"_sorted.bed ]; then
        ...
        awk -F '\t' '{print $1"\t"$2"\t"$3"\t"$10"\t"$11}' "$beds_path"/"$filename" | \
