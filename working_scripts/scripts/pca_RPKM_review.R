...
allrna_data_normalized <- zscores_all |>
  dplyr::select(all_of(PCA_20_SELECT_FEATURES), Dataset) |>
  na.omit() |>
  mutate(RPKM_sum = RPKM_tissue + RPKM_primary.cell)


ggplot(allrna_data_normalized |> 
         dplyr::filter(Dataset%in% c("short-ncrna","short-ncrna-negative-control")), 
       aes(x=RPKM_sum, colour = Dataset)) +
  geom_density(linewidth = 1.5) +
  scale_x_log10(breaks=c(0.01,0.1,1,10,100), labels = c(0.01,0.1,1,10,100)
  ) +
  scale_color_manual(values = c("short-ncrna-negative-control" = "#56bdfcFF", "short-ncrna" = "#D6604DFF"),
                     labels = c("sncRNA(+)","sncRNA(-)")) +
  labs(subtitle="sncRNA", x = "RPKM sum", y = "Density") + 
  theme_minimal() +
  theme(
...
allrna_pca_result <- prcomp(allrna_data_normalized |> dplyr::select(-Dataset,-`Tissue RPKM`,-`Primary cell RPKM`) |> scale(), center = TRUE, rank. = 7)
...
# Change colnames
colnames(allrna_data_normalized) <- c(PCA_20_SELECT_FEATURES_LABELS, "Dataset", "RPKM sum")

# Compute contribution of features to variation of PCAs
allrna_pca <- princomp(allrna_data_normalized |> dplyr::select(-Dataset,-`Tissue RPKM`,-`Primary cell RPKM`) |> scale())
summary(allrna_pca)

# Inspect variable loading
allrna_pca$loadings[, 1:7]
...
p <- fviz_cos2(allrna_pca, choice = "var", axes = 7, title = "Contribution of Features to PC 7 Variance - all RNA")

# 2. Modify the plot object's aesthetics
p_modified <- p + labs(y = "Contribution to PC7 Variance") + theme(text = element_text(size = 26),
...
fviz_pca_var(allrna_pca,
             axes = c(1, 7), # This specifies plotting PC1 vs PC6
             col.var = "cos2",
             gradient.cols = c("black", "orchid", "blue"),
             repel = TRUE,
             title = "Biplot - All RNA",
...
pc1_dist_plot <- ggplot(protein_pc_scores, aes(x = PC1, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC1 (22.6%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.3)) +
  theme_minimal() +
  theme(
    plot.subtitle = element_text(size = 38, hjust = 0.5),
...
pc2_dist_plot <- ggplot(protein_pc_scores, aes(x = PC2, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC2 (12.3%)", y = "Density") + 
  coord_cartesian(c(-7.5, 5),
                  c(0, 0.35)) +
  theme_minimal() +
  theme(
    plot.subtitle = element_text(size = 38, hjust = 0.5),
...
pc3_dist_plot <- ggplot(protein_pc_scores, aes(x = PC3, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC3 (7.7%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.5)) +
  theme_minimal() +
  theme(
...
pc4_dist_plot <- ggplot(protein_pc_scores, aes(x = PC4, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC4 (7.0%)", y = "Density") + 
  coord_cartesian(c(-5, 6),
                  c(0, 0.45)) +
  theme_minimal() +
  theme(
    plot.subtitle = element_text(size = 38, hjust = 0.5),
...
pc5_dist_plot <- ggplot(protein_pc_scores, aes(x = PC5, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC5 (6.2%)", y = "Density") + 
...
pc6_dist_plot <- ggplot(protein_pc_scores, aes(x = PC6, color = `Gene type`)) +
  geom_density(size = 1.5) + # Using a single geom_density is more efficient
  scale_color_manual(values = c("mRNA(-)" = "#72b6e7", "mRNA(+)" = "#F4A582FF")) +
  labs(subtitle="mRNA", x = "PC6 (5.3%)", y = "Density") + 
  coord_cartesian(c(-2, 4),
                  c(0, 1.5)) +
  theme_minimal() +
  theme(
    plot.subtitle = element_text(size = 38, hjust = 0.5),
...
# Plot PC distributions
pc1_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC1, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC1 (22.6%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.3)) +
  theme_minimal() +
  theme(
...
pc2_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC2, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC2 (12.3%)", y = "Density") + 
  coord_cartesian(c(-7.5, 5),
                  c(0, 0.35)) +
  theme_minimal() +
  theme(
...
pc3_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC3, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC3 (7.7%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.5)) +
  theme_minimal() +
  theme(
...
pc4_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC4, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC4 (7.0%)", y = "Density") + 
  coord_cartesian(c(-5, 6),
                  c(0, 0.45)) +
  theme_minimal() +
  theme(
...
pc5_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC5, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC5 (6.2%)", y = "Density") + 
  coord_cartesian(c(-5, 5),
                  c(0, 0.5)) +
  theme_minimal() +
  theme(
...
pc6_dist_plot <- ggplot(lncrna_pc_scores, aes(x = PC6, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("lncRNA(-)" = "#53a4f5FF", "lncRNA(+)" = "#e37b88FF")) +
  labs(subtitle="lncRNA", x = "PC6 (5.3%)", y = "Density") + 
  coord_cartesian(c(-2, 4),
                  c(0, 1.5)) +
  theme_minimal() +
  theme(
...
lims <- c(range(sncrna_pc_scores$PC1), range(sncrna_pc_scores$PC7))
...
density_func_sncrna <- kde2d(sncrna_pc_scores_pos$PC1, sncrna_pc_scores_pos$PC7, n = 80, lims = lims)
density_neg_sncrna <- kde2d(sncrna_pc_scores_neg$PC1, sncrna_pc_scores_neg$PC7, n = 80, lims = lims)
...
filled.contour(density_func_sncrna, xlab = "PC1", ylab = "PC7", main = "Filled Contour Plot of PCA Density")
filled.contour(density_neg_sncrna, xlab = "PC1", ylab = "PC7", main = "Filled Contour Plot of PCA Density")
...
df_density_neg_sncrna$z <- as.vector(density_neg_sncrna$z)
df_density_pos_sncrna <- expand.grid(PC1 = density_func_sncrna$x, PC7 = density_func_sncrna$y)
df_density_pos_sncrna$z <- as.vector(density_func_sncrna$z)
df_density_neg_sncrna <- expand.grid(PC1 = density_neg_sncrna$x, PC7 = density_neg_sncrna$y)
df_density_neg_sncrna$z <- as.vector(density_neg_sncrna$z)
...
# Plot PC distributions
pc1_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC1, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC1 (22.6%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.3)) +
  theme_minimal() +
  theme(
...
pc2_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC2, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC2 (12.3%)", y = "Density") + 
  coord_cartesian(c(-7.5, 5),
                  c(0, 0.35)) +
  theme_minimal() +
  theme(
...
pc3_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC3, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC3 (7.7%)", y = "Density") + 
  coord_cartesian(c(-10, 5),
                  c(0, 0.5)) +
  theme_minimal() +
  theme(
...
pc4_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC4, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC4 (7.0%)", y = "Density") + 
  coord_cartesian(c(-5, 6),
                  c(0, 0.45)) +
  theme_minimal() +
  theme(
...
pc5_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC5, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC5 (6.2%)", y = "Density") + 
  coord_cartesian(c(-5, 5),
                  c(0, 0.5)) +
  theme_minimal() +
  theme(
...
pc6_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC6, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC6 (5.3%)", y = "Density") + 
  coord_cartesian(c(-2, 4),
                  c(0, 1.5)) +
  theme_minimal() +
  theme(
...
pc7_dist_plot <- ggplot(sncrna_pc_scores, aes(x = PC7, color = `Gene type`)) +
  geom_density(size = 1.5) +
  scale_color_manual(values = c("sncRNA(-)" = "#0491e8", "sncRNA(+)" = "#D6604DFF")) +
  labs(subtitle="sncRNA", x = "PC7 (5.3%)", y = "Density") + 
  coord_cartesian(c(-2, 4),
                  c(0, 1.5)) +
  theme_minimal() +
  theme(
...
# Plot PCA
pca_sncrna_plot <- ggplot(sncrna_pc_scores, aes(x = PC1, y = PC7, shape = `Gene type`, color = `Gene type`)) +
  geom_point(data = subset(sncrna_pc_scores, `Gene type` == "sncRNA(-)"),
             aes(x = PC1, y = PC7), shape = 4, size = 4) +
  # Add semi-transparent density contours
  geom_contour(data = df_density_neg_sncrna, aes(x = PC1, y = PC7, z = z), 
               bins= 20, color = "#0491e8", alpha = 0.6, linewidth = 1) +
  geom_point(data = subset(sncrna_pc_scores, `Gene type` == "sncRNA(+)"),
             aes(x = PC1, y = PC7), shape = 17, size = 4) +
  # Add semi-transparent density contours for sncRNA
  geom_contour(data = df_density_pos_sncrna, aes(x = PC1, y = PC7, z = z), 
               bins = 20, color = "#471810", alpha = 0.6, linewidth = 1) +
  # Use scale_manual functions
  scale_color_manual(values = my_colors) +
  scale_shape_manual(values = my_shapes) +
  labs(subtitle = "sncRNA", x = "PC1 (22.6%)", y = "PC7 (5.3%)") +
  theme_minimal()+
  theme(
    plot.subtitle = element_text(size = 38, hjust = 0.5),  # Increase title size
    axis.title = element_text(size = 30),  # Increase axis title size
  ...
  ylim(-5, 5)
pca_sncrna_plot
ggsave(paste0(PCA_SNCRNA_20_FEATURES_LOADINGS_PLOT_FILE,"_pc7.png"), pca_sncrna_plot, scale = 3, width = 3840, height = 2160, units = "px", bg = "white", dpi = 600)
